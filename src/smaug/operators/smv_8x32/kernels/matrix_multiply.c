...
void smv_matrix_multiply_transpose_nc_vec_fxp(float16* host_a,
    ...
    // Producing output by rows, same number as a_height 
    a_act:
    for (int a_act = 0; a_act < a_height; a_act++) {
        // Rows are looked by groups of PE (every row one PE)
        b_row:
        for (int b_row = 0; b_row < b_height; b_row += NUM_PE_INSTS) {
	        // Load partial results if needed
            ...
            // Columns are looked by groups of NUM_MACC_INSTS 
            b_col:
            for (int b_col = 0; b_col < b_col_sample; b_col += NUM_MACC_INSTS) {
                // Load a row, NUM_MACC_INSTS at time
                v8fp_t a_reg[NUM_MACC_INSTS];
                a_reg_load:
                for (int a_vec = 0; a_vec < NUM_MACC_INSTS; a_vec++) {
                    int a_col = a_start / VECTOR_SIZE + b_col + a_vec;
                    // If inside of a load it, if outside 'pad' with zero
                    a_reg[a_vec] =
                            a_col >= a_width_vec ? zero : _a[a_act][a_col];
                }
                // Loading b_reg: all PEs are initialized at zero full width 
                v8fp_t b_reg[NUM_PE_INSTS][NUM_MACC_INSTS] = {
                    { zero, zero, zero, zero },
                    { zero, zero, zero, zero },
                    { zero, zero, zero, zero },
                    { zero, zero, zero, zero },
                ...
                // Loading PEs with b values (2nd matrix)
                load_b_pe:
                for (int pe_id = 0; pe_id < NUM_PE_INSTS; pe_id++) {
                    // Loading multipliers of a PE
                    load_b_mu:
                    for (int macc_idx = 0; macc_idx < NUM_MACC_INSTS; 
                            macc_idx++) {
                                // One PE is assigned to one row
                                int pe_row = b_row + pe_id;
                                // Select the right 8 wide vector
                                int this_b_col = b_col + macc_idx;
                                // Fill the register of the PE, 1 vector at time
                                // If out of rows or out of cols 'pad' with zero
                                b_reg[pe_id][macc_idx] = 
                                    (pe_row >= b_height || 
                                     this_b_col >= b_width_vec) 
                                        ? zero : _b[pe_row][b_col + macc_idx];
                                }
                }
                // Register for the multiplication (full width)
                v8fp_t product_reg[NUM_PE_INSTS][NUM_MACC_INSTS];
                // Register for 1th reduction (NUM_MACC_INSTS to one vector)
                v8fp_t accum_vec_reg[NUM_PE_INSTS] = {
                    zero, zero, zero, zero, zero, zero, zero, zero
                };
                // Register 2nd reduction (reduce one vector to a value)
                
                // Reduce means: sum all the values on given dimension to 
                // reduce the number dimensions.
                ...
                for (int pe_id = 0; pe_id < NUM_PE_INSTS; pe_id++) {
                    ...
                    for (int macc_idx = 0; macc_idx < NUM_MACC_INSTS; 
                            macc_idx++) {
                                product_reg[pe_id][macc_idx] =
                                    a_reg[macc_idx] * b_reg[pe_id][macc_idx];
                    }
                    // Reduce NUM_MACC_INSTS vectors to one vector
                    reduce_1:
                    for (int macc_idx = 0; macc_idx < NUM_MACC_INSTS; 
                            macc_idx++) {
                                accum_vec_reg[pe_id] += 
                    ...
                    // Reduce one vector to one value
                    reduce_2:
                    for (int vec_i = 0; vec_i < VECTOR_SIZE; vec_i++) {
                        accum_reg[pe_id] += accum_vec_reg[pe_id][vec_i];
                    }
                    partial_sums[pe_id] += accum_reg[pe_id];
                }
            }
